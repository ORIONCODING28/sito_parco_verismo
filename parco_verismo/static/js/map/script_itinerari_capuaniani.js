document.addEventListener("DOMContentLoaded", function() {
  // Inizializza mappa con stile personalizzato - centrata su Mineo
  var map = L.map(document.querySelector(".map-container"), { 
    zoomControl: false,
    scrollWheelZoom: false
  }).setView([37.2648, 14.6925], 11); // Centro su Mineo
  ...
  // Color helper: get CSS var (fallback al colore secondary del tema - oro)
  function getColorVar(varName) {
    const v = getComputedStyle(document.documentElement).getPropertyValue(varName);
    return (v || '#BF9B30').trim();
  }
  const MAIN_ROUTE_COLOR = getColorVar('--color-secondary');
  ...
  document.querySelector('.reset-view-btn')?.addEventListener('click', function() {
    map.setView([37.2648, 14.6925], 11);
  });
});
